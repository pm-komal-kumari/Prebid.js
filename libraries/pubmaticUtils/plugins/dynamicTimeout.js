...
export function processBidRequest(reqBidsConfigObj) {
  ...
  if (shouldThrottle(timeoutConfig?.config?.skipRate !== undefined ? timeoutConfig?.config?.skipRate : CONSTANTS.DEFAULT_SKIP_RATE)) {
    logInfo(`${CONSTANTS.LOG_PRE_FIX} Dynamic timeout is skipped (skipRate: ${timeoutConfig?.config?.skipRate !== undefined ? timeoutConfig?.config?.skipRate : CONSTANTS.DEFAULT_SKIP_RATE}%)`);
